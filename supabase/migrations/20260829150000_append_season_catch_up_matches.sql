-- Append catch-up league matches for a newly enrolled team (round-robin seasons only).
...
BEGIN
  ...
  IF v_format = 'groups_knockout' THEN
    SELECT COUNT(*) INTO v_knockout_rounds
    FROM public.season_knockout_rounds skr
    WHERE skr.season_id = p_season_id;

    IF v_knockout_rounds > 0 THEN
      RAISE EXCEPTION 'Cannot append catch-up matches after knockout phase started'
        USING ERRCODE = 'P0001';
    END IF;
  END IF;
