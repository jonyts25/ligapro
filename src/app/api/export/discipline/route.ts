...
export async function GET(request: NextRequest) {
  ...
  if (format === "pdf") {
    const pdf = await buildDisciplinePdf(ctx);
    return pdfResponse(pdf, `disciplina-${slug}.pdf`);
  }

  const csv = await buildDisciplineCsv(ctx);
  return csvResponse(csv, `disciplina-${slug}.csv`);
}
