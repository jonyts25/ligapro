...
export default async function OrganizationDisciplineHubPage({
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <PageHeader
        title="Disciplina"
        description="Elige una temporada para ver tarjetas, suspensiones y sanciones."
      />
      {!seasons.length ? (
        <EmptyState
          title="Sin temporadas"
          description="La disciplina se registra por temporada cuando hay partidos y eventos."
        />
      ) : (
          ...
          {seasons.map((s) => (
            <li key={s.seasonId}>
              <Card className="flex flex-wrap items-center justify-between gap-3">
                ...
                <Link
                  href={`/organizaciones/${organizationId}/torneos/${s.competitionId}/temporadas/${s.seasonId}/disciplina`}
