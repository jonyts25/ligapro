...
export async function getSeasonTeams(
  ...
  return seasonTeams.map((st) => {
    ...
    return {
      ...
      status_effective_at: st.status_effective_at ?? "",
...
export async function getSeasonTeamRoster(
  ...
  const { data: seasonTeam } = await supabase
    .from("season_teams")
    .select(
      "id, season_id, team_id, organization_id, display_name, group_name, registration_status, status, status_effective_at, teams(name), seasons(id, name, competition_id, competitions(id, name))"
    )
    .eq("id", seasonTeamId)
    .eq("season_id", seasonId)
    .eq("organization_id", organizationId)
  ...
  return {
    ...
    status_effective_at: seasonTeam.status_effective_at ?? "",
    teamName: teamName ?? "Equipo",
